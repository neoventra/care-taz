@extends('layouts.app')

@section('title', 'Companionship Care Nottingham | Caretaz Healthcare')
@section('description', 'Companionship Care in Nottingham from Caretaz Healthcare — warm social visits that fight loneliness with conversation, outings, hobbies and gentle routine prompts.')
@section('body_class', 'sd')
@push('styles')
  <link rel="stylesheet" href="{{ versioned_asset('css/service-detail.css') }}">
@endpush

@section('content')
<main>
<section class="sd-hero" aria-labelledby="sd-hero-heading">
      <div class="sd-hero__bg" style="background-image:url('../assets/images/care-banner-companionship.jpg')" aria-hidden="true"></div>
      <div class="sd-hero__inner">
        <nav class="breadcrumbs" aria-label="Breadcrumb">
          <a href="{{ route('home') }}">Home</a> <span>/</span>
          <a href="{{ route('home') }}">Services</a> <span>/</span>
          <span>Companionship Care</span>
        </nav>
        <p class="sd-brand">Caretaz Healthcare · Warm Company at Home</p>
        <h1 id="sd-hero-heading">Company that lifts the <em>day</em>.</h1>
        <p class="sd-hero__lead">Companionship care across Nottingham — regular social visits that fight loneliness with conversation, shared interests and the reassurance of a familiar face.</p>
        <div class="btn-row">
          <a class="btn btn--teal" href="#appointment">Book Assessment</a>
          <a class="btn btn--outline" href="tel:03330344121">Call 0333 034 4121</a>
        </div>
      </div>
    </section>

    <section class="sd-trust">
      <div class="container">
        <div class="sd-trust__grid">
          <div class="sd-trust__card reveal">
            <strong>Social wellbeing</strong>
            <span class="sd-trust__label">Connection that matters</span>
          </div>
          <div class="sd-trust__card reveal">
            <strong>CQC</strong>
            <span class="sd-trust__label">Fully regulated</span>
          </div>
          <div class="sd-trust__card reveal">
            <strong>Nottingham</strong>
            <span class="sd-trust__label">City &amp; surrounding areas</span>
          </div>
          <div class="sd-trust__card reveal">
            <strong>Regular visits</strong>
            <span class="sd-trust__label">Consistent, familiar faces</span>
          </div>
        </div>
      </div>
    </section>

    <section class="sd-section" aria-labelledby="about-service">
      <div class="container">
        <div class="sd-split">
          <div class="sd-split__media reveal-left">
            <img src="{{ asset('assets/images/care-banner-companionship.jpg') }}" alt="Companionship Care social visit with Caretaz Healthcare" width="720" height="540" loading="eager">
            <div class="sd-split__float">
              <strong>Presence</strong>
              <span>Warm company that lasts</span>
            </div>
          </div>
          <div class="sd-split__copy reveal-right">
            <p class="eyebrow">About this service</p>
            <h2 id="about-service">Visits that fight loneliness — gently.</h2>
            <p>Companionship care from Caretaz Healthcare is for people who need more than an empty afternoon. Our carers bring conversation, shared hobbies, short outings and mealtime company — warm social support that lifts mood without turning the visit into heavy clinical care.</p>
            <p>Loneliness can quietly erode confidence. Regular visits with consistent faces restore dignity, routine and something to look forward to — whether that is a walk to the shops, a favourite game or simply someone to talk to over a cup of tea.</p>
            <p>Where personal care is also needed, companionship can sit alongside hourly care under the same Nottingham team — so wellbeing and practical help stay joined up.</p>
            <div class="sd-pill-row">
              <span class="sd-pill"><i></i> Conversation</span>
              <span class="sd-pill"><i></i> Outings</span>
              <span class="sd-pill"><i></i> Hobbies &amp; games</span>
              <span class="sd-pill"><i></i> Welfare checks</span>
            </div>
            <div class="btn-row">
              <a class="btn btn--navy" href="{{ route('appointment') }}">Make Appointment</a>
              <a class="btn btn--outline-teal" href="{{ route('home') }}">All Services</a>
            </div>
          </div>
        </div>
      </div>
    </section>

    <section class="sd-section sd-section--fog" aria-labelledby="includes-heading">
      <div class="container">
        <div class="sd-head reveal">
          <p class="eyebrow">What’s included</p>
          <h2 id="includes-heading">Support shaped around connection</h2>
          <p>Every package is personalised after assessment — these are the building blocks families ask for most.</p>
        </div>
        <div class="sd-includes">
          <article class="sd-include reveal">
            <span class="sd-include__label">Connection</span>
            <h3>Conversation &amp; company</h3>
            <p>Genuine chat, listening and shared time that makes the day feel less empty.</p>
          </article>
          <article class="sd-include reveal">
            <span class="sd-include__label">Outings</span>
            <h3>Short trips out</h3>
            <p>Accompanied walks, café visits or local errands that keep life open and social.</p>
          </article>
          <article class="sd-include reveal">
            <span class="sd-include__label">Interest</span>
            <h3>Hobbies &amp; games</h3>
            <p>Cards, puzzles, reading, gardening or music — whatever lights someone up.</p>
          </article>
          <article class="sd-include reveal">
            <span class="sd-include__label">Nutrition</span>
            <h3>Mealtime company</h3>
            <p>Sitting together at meals so eating feels sociable, not solitary.</p>
          </article>
          <article class="sd-include reveal">
            <span class="sd-include__label">Wellbeing</span>
            <h3>Welfare checks</h3>
            <p>A reassuring presence that notices mood, appetite and how someone is coping.</p>
          </article>
          <article class="sd-include reveal">
            <span class="sd-include__label">Routine</span>
            <h3>Gentle routine prompts</h3>
            <p>Light reminders that support the day without taking over independence.</p>
          </article>
        </div>
      </div>
    </section>

    <section class="sd-section sd-section--ink" aria-labelledby="why-heading">
      <div class="container">
        <div class="sd-head reveal">
          <p class="eyebrow" style="color:var(--teal);">Why families choose this</p>
          <h2 id="why-heading">Impact you can feel at home</h2>
          <p>More than a service label — a standard of how we show up.</p>
        </div>
        <div class="sd-values">
          <article class="sd-value reveal">
            <div class="sd-value__num">01</div>
            <h3>Consistent faces</h3>
            <p>Regular carers so relationships can grow and visits feel familiar.</p>
          </article>
          <article class="sd-value reveal">
            <div class="sd-value__num">02</div>
            <h3>Dignity first</h3>
            <p>Company that respects pace, preferences and privacy — never patronising.</p>
          </article>
          <article class="sd-value reveal">
            <div class="sd-value__num">03</div>
            <h3>Lifts mood</h3>
            <p>Social contact that reduces isolation and gives the week a brighter shape.</p>
          </article>
          <article class="sd-value reveal">
            <div class="sd-value__num">04</div>
            <h3>Combine with care</h3>
            <p>Add personal care visits under the same provider when needs grow.</p>
          </article>
        </div>
      </div>
    </section>

    <section class="sd-section" aria-labelledby="process-heading">
      <div class="container">
        <div class="sd-head reveal">
          <p class="eyebrow">How it starts</p>
          <h2 id="process-heading">Three clear steps to care</h2>
          <p>No jargon. No pressure — just a path that feels human.</p>
        </div>
        <div class="sd-process">
          <article class="sd-step reveal">
            <div class="sd-step__num">01</div>
            <h3>Make contact</h3>
            <p>Call, email or book online. Tell us what you need and how soon.</p>
          </article>
          <article class="sd-step reveal">
            <div class="sd-step__num">02</div>
            <h3>Assessment</h3>
            <p>We meet at home (or virtually) to understand interests, routines and goals.</p>
          </article>
          <article class="sd-step reveal">
            <div class="sd-step__num">03</div>
            <h3>Care begins</h3>
            <p>Agree the plan, meet your companions, and start visits that feel settled.</p>
          </article>
        </div>
      </div>
    </section>

    <section class="sd-section sd-section--fog" aria-labelledby="faq-heading">
      <div class="container">
        <div class="sd-head reveal">
          <p class="eyebrow">Good to know</p>
          <h2 id="faq-heading">Questions families ask first</h2>
          <p>Straight answers before you book.</p>
        </div>
        <div class="sd-faq">
          <details class="reveal">
            <summary>What does companionship care include?</summary>
            <div class="sd-faq__body">Conversation, shared hobbies or games, short outings, mealtime company, welfare checks and gentle prompts with daily routines — focused on social connection rather than heavy clinical care.</div>
          </details>
          <details class="reveal">
            <summary>Is companionship care the same as personal care?</summary>
            <div class="sd-faq__body">No. Companionship centres on company and wellbeing. If personal care is also needed, we can combine visits or add hourly care under the same CQC-registered provider.</div>
          </details>
          <details class="reveal">
            <summary>How often can companionship visits take place?</summary>
            <div class="sd-faq__body">As often as feels right — from a weekly social call to several regular visits. We match faces and schedules so relationships can grow consistently.</div>
          </details>
          <details class="reveal">
            <summary>Is companionship care CQC-regulated?</summary>
            <div class="sd-faq__body">Yes. Companionship visits from Caretaz are delivered under our CQC registration by vetted, trained team members across Nottingham.</div>
          </details>
        </div>
      </div>
    </section>

    <section class="sd-section" aria-labelledby="related-heading">
      <div class="container">
        <div class="sd-head reveal">
          <p class="eyebrow">Explore further</p>
          <h2 id="related-heading">Related care pathways</h2>
          <p>Many families combine services as needs evolve.</p>
        </div>
        <div class="sd-related">
          <a class="reveal" href="{{ route('services.show', 'hourly-care') }}">
            <div class="sd-related__media"><img src="{{ asset('assets/images/care-garden-outdoors.jpg') }}" alt="Hourly Care" width="640" height="360" loading="lazy"></div>
            <div class="sd-related__body">
              <span class="sd-related__cat">Visiting</span>
              <h3>Hourly Care</h3>
            </div>
          </a>
          <a class="reveal" href="{{ route('services.show', 'holiday-companionship') }}">
            <div class="sd-related__media"><img src="{{ asset('assets/images/care-meal-support.jpg') }}" alt="Holiday Companionship" width="640" height="360" loading="lazy"></div>
            <div class="sd-related__body">
              <span class="sd-related__cat">Travel breaks</span>
              <h3>Holiday Companionship</h3>
            </div>
          </a>
          <a class="reveal" href="{{ route('services.show', 'housekeeping-services') }}">
            <div class="sd-related__media"><img src="{{ asset('assets/images/service-cleaning.jpg') }}" alt="Housekeeping Services" width="640" height="360" loading="lazy"></div>
            <div class="sd-related__body">
              <span class="sd-related__cat">Domestic</span>
              <h3>Housekeeping Services</h3>
            </div>
          </a>
        </div>
      </div>
    </section>

    <section class="sd-section sd-section--fog">
      <div class="container">
        <div class="cqc-banner reveal-zoom">
          <img src="{{ asset('assets/images/cqc.png') }}" alt="Care Quality Commission">
          <div>
            <h3>Fully Regulated by The Care Quality Commission (CQC)</h3>
            <p>Caretaz Healthcare is registered with and regulated by the CQC, the independent body overseeing health and social care in England, with our practices routinely assessed against national standards.</p>
            <p>We're also proudly registered with the ICO, reflecting our commitment to protecting your personal information.</p>
          </div>
          <div class="badge-row"><img src="{{ asset('assets/images/ico.png') }}" alt="ICO registered"></div>
        </div>
      </div>
    </section>

    <section class="sd-section">
      <div class="container">
        <div class="sd-finale reveal">
          <div>
            <p class="eyebrow" style="color:var(--teal);">Ready when you are</p>
            <h2>Ready for company that brightens the week?</h2>
            <p>Tell us what a good visit looks like — we’ll propose regular companionship that fights loneliness with warmth and consistency.</p>
            <div class="btn-row">
              <a class="btn btn--teal" href="{{ route('appointment') }}">Make an Appointment</a>
              <a class="btn btn--outline" href="tel:03330344121">Call 0333 034 4121</a>
            </div>
          </div>
          <div class="sd-finale__media">
            <img src="{{ asset('assets/images/care-garden-outdoors.jpg') }}" alt="Companionship Care in Nottingham" width="560" height="360" loading="lazy">
          </div>
        </div>
      </div>
    </section>

    <section class="section section--mist" id="appointment">
      <div class="container appoint-grid">
        <div class="reveal">
          <p class="eyebrow">Make appointment</p>
          <h2>Book an assessment</h2>
          <p>Share a few details and we’ll arrange a conversation as soon as possible. For urgent needs, please telephone.</p>
          <p><strong>Phone:</strong> <a href="tel:03330344121">0333 034 4121</a><br>
             <strong>Email:</strong> <a href="mailto:info@caretazhealthcare.co.uk">info@caretazhealthcare.co.uk</a><br>
             <strong>Address:</strong> Suite B/105, Blenheim Court, 86-88 Mansfield Road, Nottingham NG1 3HD</p>
        </div>
        <form class="form-card reveal" data-appointment-form>
          <div class="form-grid">
            <div class="form-field"><label for="name">Name</label><input id="name" name="name" required autocomplete="name"></div>
            <div class="form-field"><label for="email">Email</label><input id="email" name="email" type="email" required autocomplete="email"></div>
            <div class="form-field"><label for="phone">Phone</label><input id="phone" name="phone" required autocomplete="tel"></div>
            <div class="form-field"><label for="service">Select Services</label>
              <select id="service" name="service" required>
                <option>Urgent Care</option>
                <option>Hourly Care</option>
                <option>Overnight Care</option>
                <option>24-Hour Care</option>
                <option>Dementia Care</option>
                <option>Parkinson's Care</option>
                <option>Stroke Care</option>
                <option>Cancer Care</option>
                <option>Palliative / End-of-Life Care</option>
                <option>Supported Living</option>
                <option>Live-In Care For Couples</option>
                <option>Hospital to Home Care</option>
                <option selected>Companionship Care</option>
                <option>Carer Respite</option>
                <option>Holiday Companionship</option>
                <option>Housekeeping Services</option>
                <option>Other</option>
              </select>
            </div>
            <div class="form-field form-field--full"><label for="message">Message</label><textarea id="message" name="message" placeholder="Tell us a little about the support you need…"></textarea></div>
          </div>
          <button class="btn btn--teal" type="submit" style="margin-top:.75rem;">Make Appointment</button>
          <p class="form-note">For urgent needs, please telephone 0333 034 4121.</p>
        </form>
      </div>
    </section>
</main>
@endsection

@push('scripts')
  <script src="{{ versioned_asset('js/service-detail.js') }}"></script>
@endpush
